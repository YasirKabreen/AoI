...
w= input('enter the probability w: '); % Probability of Weirless Channel State
for i = 1:num_states
    state = state_space(i, :);
 ...
 if state(2)>=2 
        next_state14(1) = 1;
        next_state14(2)=max(next_state14(2)-2,0);
    ...
    if state(2)>=2 
      ...
      next_state15(1) = min(next_state15(1) + 1, max(AoI1_values));
    else
        next_state15(1) = min(next_state15(1) + 1, max(AoI1_values));
    end
    ...
    if state(2)>=2 
      ...
      next_state16(1) = min(next_state16(1) + 1, max(AoI1_values));
      next_state16(2)=max(next_state16(2)-1,0);
    else
        next_state16(1) = min(next_state16(1) + 1, max(AoI1_values));
    end
    ...
    if state(2)>=2 
      ...
      next_state17(1) = min(next_state17(1) + 1, max(AoI1_values));
    else
        next_state17(1) = min(next_state17(1) + 1, max(AoI1_values));
    end
    ...
    if state(2)>=2 
      ...
      next_state18(1) = min(next_state18(1) + 1, max(AoI1_values));
      next_state18(2)=max(next_state18(2)-2,0);
    else
        next_state18(1) = min(next_state18(1) + 1, max(AoI1_values));
    end
 ...
 % Sensor one transitions and rewards if it is not Not Sending and Conserving Energy:
  ...
  next_idx11 = find(ismember(state_space, next_state11, 'rows'));
       next_idx12 = find(ismember(state_space, next_state12, 'rows'));
          next_idx13 = find(ismember(state_space, next_state13, 'rows'));
             next_idx14 = find(ismember(state_space, next_state14, 'rows'));
                next_idx15 = find(ismember(state_space, next_state15, 'rows'));
                   next_idx16 = find(ismember(state_space, next_state16, 'rows'));
                      next_idx17 = find(ismember(state_space, next_state17, 'rows'));
                         next_idx18 = find(ismember(state_space, next_state18, 'rows'));
    ...
    P1(i, next_idx11) = P1(i, next_idx11)+ w*e*q;
    P1(i, next_idx12) = P1(i, next_idx12)+w*(1-e)*q;
    P1(i, next_idx13) = P1(i, next_idx13)+(1-w)*e*(q);
    P1(i, next_idx14) = P1(i, next_idx14)+(1-w)*(1-e)*(q);
    P1(i, next_idx15) = P1(i, next_idx11)+ w*e*(1-q);
    P1(i, next_idx16) = P1(i, next_idx12)+w*(1-e)*(1-q);
    P1(i, next_idx17) = P1(i, next_idx13)+(1-w)*e*(1-q);
    P1(i, next_idx18) = P1(i, next_idx14)+(1-w)*(1-e)*(1-q);
