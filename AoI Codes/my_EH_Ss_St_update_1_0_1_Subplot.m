...
% Iterate over different values of successful transmission probability p
p_values = 0:0.2:1;
for p_idx = 1:numel(p_values)
    p = p_values(p_idx);
...
for i = 1:num_states
    ...
    if state(2) > 0 % If battery is greater than 0
        ...
        P1(i, ismember(state_space, next_state11, 'rows')) = q * p *  e                +P1(i, ismember(state_space, next_state11, 'rows'));
        P1(i, ismember(state_space, next_state12, 'rows')) = q * p * (1-e)             +P1(i, ismember(state_space, next_state12, 'rows'));
        P1(i, ismember(state_space, next_state13, 'rows')) = (q * (1-p) *  e) + ((1-q) * p *  e) +((1-q) * (1-p) *  e)                      +P1(i, ismember(state_space, next_state13, 'rows'));
        P1(i, ismember(state_space, next_state14, 'rows')) = (q * (1-p) *  (1-e)) + ((1-q) * p *  (1-e)) +((1-q) * (1-p) *  (1-e))          +P1(i, ismember(state_space, next_state14, 'rows'));
