...
num_states = numel(Sk1)*numel(Sk1)*numel(AoI1_values) * numel(AoI2_values) ;
    ...
    for sk2 = Sk2
        for aoi1 = AoI1_values
             for aoi2 = AoI2_values
                        ...
                        state_space(idx, :) = [sk1,sk2,aoi1, aoi2];
                        idx = idx + 1;
             end
        end
    end
...
for i = 1:num_states
    ...
    %send with probabilty p1*q.
        next_state11(1) = 1;
        next_state11(2) = min(next_state11(2) + 1, max(AoI2_values));
     %not send with probabilty 1-(p1*q).
  ...
  next_state21 = state;
    ...
    %send with probabilty p2*q.
        next_state21(2) = 1;
        next_state21(1) = min(next_state21(1) + 1, max(AoI1_values));
     %not send with probabilty 1-(p2*q).
    ...
    %send with probabilty p3*q.
        next_state31(1) = 1;
        next_state31(2) =1;
    
          %not send with probabilty p*p.
    ...
    P1(i, next_idx11) = p1*q;
    P1(i, next_idx12) = 1-(p1*q);
    P2(i, next_idx21) = p2*q;
    P2(i, next_idx22) = 1-(p2*q);
     P3(i, next_idx31) = (p3*q);
      P3(i, next_idx32) = 1-(p3*q);
  
 

   costA1=1/2*(state(3)+1+state(4)+1)-1/2*(((0.6)^state(1))*0.9*state(3));
     costA2=1/2*(state(3)+1+state(4)+1)-1/2*(((0.6)^state(2))*0.9*state(4));
       costA3=1/2*(state(3)+1+state(4)+1)-1/2*(((0.4)^state(2))*0.9*state(4)+((0.4)^state(2))*0.9*state(4));
     ee(i)=costA2;
     e(i)=costA1;
        eee(i)=costA3;
...
for i = 1:numel(policy)
    ...
    color = colors{action};
    scatter(state_space(i, 3), state_space(i, 4), 50, marker, 'filled', 'MarkerEdgeColor', color);
