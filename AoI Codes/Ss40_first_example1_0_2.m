...
for i = 1:num_states
  ...
  next_state21 = state;
    ...
    P2(i, next_idx22) = 1-(p*q);
     P3(i, next_idx31) = ((1-p)*q);
      P3(i, next_idx32) = 1-((1-p)*q);
  
 

   costA1=1/2*(state(1)+1+state(2)+1)-1/2*(((p))*q*state(1));
     costA2=1/2*(state(1)+1+state(2)+1)-1/2*(((p))*q*state(2));
       costA3=1/2*(state(1)+1+state(2)+1)-1/2*(((1-p))*q*state(1)+((1-p))*q*state(2));
     ee(i)=costA2;
     e(i)=costA1;
        eee(i)=costA3;
