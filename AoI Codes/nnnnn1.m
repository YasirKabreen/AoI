...
discount=0.95;
p = input('Enter the probability of coverage  p: ');
q = 1;
...
Energy_cost2= B-b2;
Energy_cost3= B-b3;
Energy_saved=(b1+b2+b3)/3;

    % Cost computation with battery penalty
    Cost1 = w1*((delta1 + delta2)/2 + 1 - 0.5 * p * q * delta1) + w2*(Energy_cost1);
    Cost2 = w1*((delta1 + delta2)/2 + 1 - 0.5 * p * q * delta2) + w2*(Energy_cost2);
    Cost3 = w1*((delta1 + delta2)/2 + 1 - 0.5 * (1-p) * q * (delta1 + delta2)) + w2*(Energy_cost3);
...
new_state1_eh = [1, min(delta2+1, max_AoI2), min(max(b1-1, 0)+1, B), min(b2+1, B), min(b3+1, B)];
    ...
    new_state1_eh = [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2),min(b1+1, B), min(b2+1, B), min(b3+1, B)];
end
if b2 > 0
new_state2_eh = [min(delta1+1, max_AoI1), 1, min(b1+1, B), min(max(b2-1, 0)+1, B), min(b3+1, B)];
else
    new_state2_eh= [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), min(b1+1, B), min(b2+1, B), min(b3+1, B)];
end
if b3 >0
new_state3_eh = [1, 1, min(b1+1, B), min(b2+1, B), min(max(b3-1, 0)+1, B)];
else
    new_state3_eh = [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), min(b1+1, B), min(b2+1, B), min(b3+1, B)];
end
...
new_state_fail1_eh = [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), min(max(b1-1, 0)+1, B), min(b2+1, B), min(b3+1, B)];
   ...
   new_state_fail1_eh = [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), min(b1+1, B), min(b2+1, B), min(b3+1, B)];
end
if b2 > 0

new_state_fail2_eh = [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), min(b1+1, B), min(max(b2-1, 0)+1, B), min(b3+1, B)];
else
    new_state_fail2_eh =[min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), min(b1+1, B), min(b2+1, B), min(b3+1, B)];
end

if b3 > 0
new_state_fail3_eh = [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), min(b1+1, B), min(b2+1, B), min(max(b3-1, 0)+1, B)];
else
   new_state_fail3_eh = [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), min(b1+1, B), min(b2+1, B), min(b3+1, B)];
end
...
new_state1_no_eh = [1, min(delta2+1, max_AoI2), max(b1-1, 0), b2, b3];
   ...
   new_state1_no_eh = [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), b1, b2, b3];
end
if b2 > 0
new_state2_no_eh = [min(delta1+1, max_AoI1), 1, b1, max(b2-1, 0), b3];
else
  new_state2_no_eh =   [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), b1, b2, b3];
end
if b3 > 0
new_state3_no_eh = [1, 1, b1, b2, max(b3-1, 0)];
else
  new_state3_no_eh =   [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), b1, b2, b3];
end
...
new_state_fail1_no_eh = [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), max(b1-1, 0), b2, b3];
   ...
   new_state_fail1_no_eh =  [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), b1, b2, b3];
end
if b2 > 0
new_state_fail2_no_eh = [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), b1, max(b2-1, 0), b3];
else
   new_state_fail2_no_eh =  [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), b1, b2, b3];
end
if b3 > 0
new_state_fail3_no_eh = [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), b1, b2, max(b3-1, 0)];
else
   new_state_fail3_no_eh = [min(delta1+1, max_AoI1), min(delta2+1, max_AoI2), b1, b2, b3];
end
