...
% Loop over the given probabilities
probabilities = [0.9, 0.6, 0.55];
figure;

for prob_idx = 1:length(probabilities)
    p = probabilities(prob_idx);
  ...
  next_state21 = state;
    ...
    %send with probabilty 1-p*1-p.
        next_state31(1) = 1;
        next_state31(2) =1;
     % with probabilty p(1-p).
     next_state32(1) = min(next_state32(1) + 1, max(AoI1_values));
     next_state32(2) = 1;
          %not send with probabilty 1-p*p.
     next_state33(1) = 1;
    ...
    P2(i, next_idx21) = p;
    P2(i, next_idx22) = 1-p;
    P3(i, next_idx31) = (1-p)*(1-p);
    P3(i, next_idx32) = p*(1-p);
    P3(i, next_idx33) = (1-p)*p;
    P3(i, next_idx34) = p*p;
    ...
    % Plotting part
    subplot(2, 2, prob_idx); % 1-row, 3-columns subplot structure
