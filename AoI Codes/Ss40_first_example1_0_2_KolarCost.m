...
for i = 1:num_states
    ...
    % Cost is the sum of AoI values
          % Cost for action a1 (Selecting Sensor 1)
        Cost1 = (delta1 + delta2)/2 + 1 - 0.5 * p * q * delta1;
        
        % Cost for action a2 (Selecting Sensor 2)
        Cost2 = (delta1 + delta2)/2 + 1 - 0.5 * p * q * delta2;
        
        % Cost for action a3 (Selecting Sensor 3)
        Cost3 = (delta1 + delta2)/2 + 1 - 0.5 * (1-p) * q * (delta1 + delta2);
